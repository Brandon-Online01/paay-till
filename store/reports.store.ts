...
export const useReportsStore = create<ReportsState>((set, get) => ({
    ...
    applyFiltersAndSort: () => {
        ...
        if (filters.paymentMethod.length > 0) {
            filtered = filtered.filter((t) =>
                t.paymentMethods.some((pm) =>
                    filters.paymentMethod.includes(pm.type)
                )
            );
        }
    ...
    calculateMetrics: () => {
        ...
        const largestSale =
            sales.length > 0 ? Math.max(...sales.map((t) => t.totalAmount)) : 0;
        const totalItems = sales.reduce(
            (sum, t) =>
                sum +
                t.items.reduce((itemSum, item) => itemSum + item.quantity, 0),
            0
        );
        ...
        const metrics: SalesMetrics = {
            ...
            averageItemsPerOrder:
                sales.length > 0 ? totalItems / sales.length : 0,
            totalItems,
        };
